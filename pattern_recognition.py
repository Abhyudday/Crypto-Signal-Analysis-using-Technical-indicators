    ...
    def analyze(self, df: pd.DataFrame) -> Tuple[SignalType, ConfidenceLevel]:
        ...
        try:
            ...
            if self._check_bullish_engulfing(last_3_candles):
                # Only add if price is below SMA50 (oversold condition)
            ...
            if self._check_bearish_engulfing(last_3_candles):
                # Only add if price is above SMA50 (overbought condition)
                if df['close'].iloc[-1] > df['sma_50'].iloc[-1]:
                    bearish_patterns.append(('bearish_engulfing', 0.8))
                    
            if self._check_morning_star(last_3_candles):
                # Only add if price is below SMA50 and showing signs of reversal
            ...
            if self._check_evening_star(last_3_candles):
                # Only add if price is above SMA50 and showing signs of reversal
                if (df['close'].iloc[-1] > df['sma_50'].iloc[-1] and 
                    df['close'].iloc[-1] < df['close'].iloc[-2]):
            ...
            # Check trend patterns (only if they're strong)
            if self._check_uptrend(df):
                # Only add if trend is strong (price significantly above both SMAs)
                if (df['close'].iloc[-1] > df['sma_20'].iloc[-1] * 1.02 and 
                    df['sma_20'].iloc[-1] > df['sma_50'].iloc[-1] * 1.02):
                    bullish_patterns.append(('uptrend', 0.6))
                    
            if self._check_downtrend(df):
                # Only add if trend is strong (price significantly below both SMAs)
                if (df['close'].iloc[-1] < df['sma_20'].iloc[-1] * 0.98 and 
                    df['sma_20'].iloc[-1] < df['sma_50'].iloc[-1] * 0.98):
            ...
            # Determine signal based on patterns with stricter conditions
            if bullish_patterns and not bearish_patterns:
                # Only return HIGH confidence if we have multiple bullish patterns
                if len(bullish_patterns) >= 2:
                    return SignalType.BUY, ConfidenceLevel.HIGH
                # Or if we have a very strong single pattern
                elif any(conf >= 0.9 for _, conf in bullish_patterns):
                    return SignalType.BUY, ConfidenceLevel.HIGH
                else:
                    return SignalType.BUY, ConfidenceLevel.MEDIUM
                    
            elif bearish_patterns and not bullish_patterns:
                # Only return HIGH confidence if we have multiple bearish patterns
                if len(bearish_patterns) >= 2:
                    return SignalType.SELL, ConfidenceLevel.HIGH
                # Or if we have a very strong single pattern
                elif any(conf >= 0.9 for _, conf in bearish_patterns):
                    return SignalType.SELL, ConfidenceLevel.HIGH
                else:
